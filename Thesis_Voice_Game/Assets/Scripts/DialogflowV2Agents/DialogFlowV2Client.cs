...
namespace Syrus.Plugins.DFV2Client
{
	...
	public class DialogFlowV2Client : MonoBehaviour 
	{
		...
		//@hoatong
		/// <summary>
		/// Makes a POST request to Dialogflow for detecting an intent from audio.
		/// </summary>
		/// <param name="talker">The ID of the entity who is talking to the bot.</param>
		/// <param name="languageCode">The language code of the request.</param>
		public void DetectIntentFromAudio(string audio64, string talker, string languageCode = "")
		{
			...
			//Debug.Log("in v2client, audio is" + audio64 + ", id is " + talker + ", lang is" + languageCode);
		...
		private IEnumerator DetectIntent(DF2QueryInput queryInput, string session, string audio ="")
		{
			...
			request.OutputAudioConfig.SynthesizeSpeechConfig = new DF2SynthesizeSpeechConfig();
			request.OutputAudioConfig.SynthesizeSpeechConfig.Voice = new DF2VoiceSelectionParams();
			request.OutputAudioConfig.SynthesizeSpeechConfig.Voice.ssmlGender = "SSML_VOICE_GENDER_FEMALE";
			...
			// Processes response.
			if (df2Request.result == UnityWebRequest.Result.ConnectionError || df2Request.result == UnityWebRequest.Result.ProtocolError)
		...
		private IEnumerator ClearSessionRequest(string session)
		{
			...
			if (deleteRequest.result == UnityWebRequest.Result.ProtocolError || deleteRequest.result == UnityWebRequest.Result.ConnectionError)
